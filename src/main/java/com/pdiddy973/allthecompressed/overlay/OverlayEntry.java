...
public class OverlayEntry {
     ...
     */
    public OverlayEntry(Identifier parent, @Nullable String registryOverride) {
        this.parent = parent;
     ...
     */
    private Supplier<BlockBehaviour.Properties> getProperties(Identifier parent) {
    ...
    private DeferredBlock<Block> block(String path, Supplier<BlockBehaviour.Properties> properties, int level) {
        Function<BlockBehaviour.Properties, Block> supplier = isPillar() ? OverlayPillarBlock::new : OverlayBlock::new;
        return ModRegistry.OVERLAY_BLOCKS.register(generateName(path, level), key -> supplier.apply(properties.get().setId(ResourceKey.create(Registries.BLOCK, key))));
    }
